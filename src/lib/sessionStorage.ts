...
export async function saveFittingSession(
  ...
  const { data: session, error } = await supabase
    ...
    .insert({
      ...
      tempo:                inputs.tempo,
      club_path_deg:        inputs.clubPathDeg ?? null,
      face_angle_deg:       inputs.faceAngleDeg ?? null,
      player_profile:       inputs.playerProfile,
      face_to_path_deg:     result.dPlane?.faceToPath ?? null,
      flight_shape:         result.dPlane?.flightShape ?? null,
